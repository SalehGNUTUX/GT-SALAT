...
/**
 * تذكيراتُ السُّنن الموسميّة حسب التاريخ الهجريّ ويوم الأسبوع — **مطابقةٌ لـ`SunnahReminders.kt`
 * في نسخة الهاتف**، فأيّ تعديلٍ هنا يُنقَل إلى هناك وإلّا اختلف التذكير بين التوأمين.
 *
 * تُعيد رسالةً **واحدةً** لليوم بأولويّة المناسبة الهجريّة الخاصّة على السُّنّة الأسبوعيّة،
 * فلا يزدحم المستخدم بأكثرَ من تذكيرٍ في اليوم.
 */
export function sunnahReminderFor(date: Date, offset = 0): { title: string; body: string } | null {
  const h = hijriParts(date, offset);
  if (!h) return null;
  const { month, day } = h;
  ...
  if (month === 1) {
    if (day === 9) return { title: 'عاشوراء', body: 'غداً عاشوراء (10 محرّم): صيامه يكفّر السنةَ الماضية. ويُستحبّ صيام التاسوعاء معه — فصُم اليوم (9 محرّم).' };
    if (day === 10) return { title: 'عاشوراء', body: 'اليومَ عاشوراء (10 محرّم): صيامه يكفّر السنةَ الماضية، صيامٌ مستحبّ.' };
    if (day === 1) return { title: 'المحرّم', body: 'شهرُ الله المحرّم: «أفضلُ الصيام بعد رمضان صيامُ شهر الله المحرّم». فأكثِر فيه من الصيام.' };
  }
  if (month === 8 && day === 1) {
    return { title: 'شعبان', body: 'دخل شعبان: كان النبيُّ ﷺ يُكثر الصيام فيه حتى يكاد يصومه كلَّه. فأكثِر من الصيام.' };
  }
  if (month === 10 && day === 2) {
    return { title: 'ستٌّ من شوّال', body: 'ستٌّ من شوّال: «مَن صام رمضان ثمّ أتبعه ستّاً من شوّال فكأنّما صام الدهر». فبادِر بها هذا الشهر.' };
  }
  if (month === 12) {
    if (day === 9) return { title: 'يوم عرفة', body: 'اليومَ يومُ عرفة (9 ذي الحجّة): صيامه لغير الحاجّ يكفّر سنتين — الماضية والباقية.' };
    if (day === 8) return { title: 'يوم عرفة', body: 'غداً يومُ عرفة (9 ذي الحجّة): صيامه لغير الحاجّ يكفّر سنتين. فاستعدَّ له.' };
    if (day >= 1 && day <= 7) return { title: 'عشر ذي الحجّة', body: 'من عشرِ ذي الحجّة: «ما من أيّامٍ العملُ الصالحُ فيهنّ أحبُّ إلى الله من هذه». أكثِر من الذكر والتكبير والصيام.' };
  }
  ...
  switch (date.getDay()) {
    case 1: // الاثنين
    case 4: // الخميس
      return { title: 'صيام الاثنين والخميس', body: 'اليومَ يُستحبّ صيامه: «تُعرَض الأعمالُ يومَ الاثنين والخميس، فأُحبُّ أن يُعرَض عملي وأنا صائم».' };
    case 5: // الجمعة
      return { title: 'سنن الجمعة', body: 'اليومَ الجمعة: اقرأ سورة الكهف، وأكثِر الصلاة على النبيّ ﷺ، وبكّر إلى الجُمعة، واغتنِم ساعةَ الإجابة.' };
    default:
      return null;
  }
}
