...
`places` (317 مدينة في 88 دولة، لاختيار الموقع دون إنترنت) · `asma` (100 اسم) · `hadith` (90 حديثاً في مجموعتَين) · `duas` (28) · `hikam` (32) · `hisn` (132 باباً، 267 ذكراً) ·
`tafsir` (114 سورة، 6236 آية بنصّها العثماني وتفسيرها، ~4 ميغابايت) · `quran_meta` · `daily_ayat` (45) ·
`purity_salah` (13 درساً مالكيّاً بمصادره) · `ruqyah` (9 أقسام) ·
...
### الموارد المحزَّمة والبروتوكول (2.2)

`gtsalat://` **بمضيفَين لا واحد** (`downloads.ts`):

| المضيف | الجذر | ما يخدمه |
|--------|-------|----------|
| `local` | `userData/quran/` | ما نزّله المستخدم: المصحف وصوت القرآن |
| `res` | `resourcesDir()` | الموارد المحزَّمة: `learn/` (233 صورة) و`audio/*.ogg` |

الموارد المحزَّمة **لا يمكن أن تمرّ عبر Vite** (خارج `src/`) **ولا عبر `file://`** (يمنعه
كروميوم من صفحةٍ على http في التطوير)، فالبروتوكول هو الطريق الوحيد العامل في الوضعين.
وكلّ مضيفٍ يحرس جذره فلا يخرج منه مسارٌ خبيث. الواجهة تبني الروابط بـ`resourceUrl` /
`galleryUrls` **في `quran.ts`** (وحدةٌ بلا تبعيات) لا في `downloads.ts` — لا تنقلهما إليها.

**مزلقٌ وقع فعلاً — البروتوكول يخدم `Range` بنفسه:** كان يفوّض إلى `net.fetch('file://…')`،
وهي تُعيد الملفّ كاملاً **بلا `Content-Length` ولا `Accept-Ranges`** وتتجاهل ترويسة `Range`
(تحقّقتُ: الاستجابة ترويستان فقط). فيراه كروميوم بثّاً مجهول الطول ⇒ `audio.duration = Infinity`
⇒ **لا شريط مدّةٍ ولا انتقالَ بالسحب** في تسجيلٍ من ثلاثٍ وستّين دقيقة — والصوت يعمل فلا يبدو
أنّ ثمّة عطباً. الآن يقرأ الحجم بنفسه ويعيد 206 مع `Content-Range`، ويضبط `Content-Type`
بالامتداد. **أيّ صوتٍ أو فيديو يُضاف مستقبلاً يعتمد على هذا** — لا تُعِد التفويض إلى `net.fetch`.

للتحقّق بعد أيّ تعديلٍ على المعالج (يختبر الوحدة الحيّة لا نسخةً يدويّة):

```bash
npx esbuild electron/downloads.ts --bundle --platform=node --format=cjs \
  --external:electron --outfile=/tmp/dl.cjs
# ثمّ نافذةٌ مخفيّةٌ تُسند src إلى gtsalat://res/audio/… وتقرأ duration وتجرّب currentTime
```

`extraResources` ينسخ `resources/` كاملاً إلى جذر `resourcesPath`، فالمسار النسبيّ
(`learn/wudu/01.webp`) واحدٌ في التطوير والإنتاج.

### الطهارة والصلاة · الرقية الشرعية (2.2)

قسمان **مدفوعان بالبيانات بالكامل**: `purity_salah.json` و`ruqyah.json` — **نفس ملفّي نسخة
الهاتف بلا تعديل**، ونماذجهما في `types.ts` مطابقةٌ لـ`LearnModels.kt` حقلاً بحقل.
**تصحيحُ نصٍّ أو حكمٍ أو مصدرٍ يكون في الـJSON لا في الشيفرة**، ثمّ يُنسَخ الملفّ إلى النسخة
الأخرى فتبقيان متطابقتَين.

**قواعد:**
1. **نصّ آيات الرقية لا يُخزَّن مرّتين**: المقطع يحمل `{surah, ayahFrom, ayahTo}` والنصّ
   يُجلَب من `tafsirSurah` — فلا نسخةٌ ثانيةٌ من القرآن ولا نصٌّ زائدٌ عبر IPC.
2. الأداتان التفاعليّتان (`QasrTool` · `TaharaTool`) تُفتحان **داخل القسم** الذي يشير إليهما
   بحقل `tool`، لا كبطاقتين مستقلّتين — كما في الهاتف.
3. الملفّان يمرّان كاملَين عبر IPC (45KB + 19KB) خلافاً للتفسير — دون حدّ ما يُثقل.

### الصوت المُضمَّن (2.2)

`resources/audio/` يضمّ الآن — إضافةً إلى أصوات الأذان — **أذكار الصباح والمساء والنوم**
ورقيةَ الشيخ سعد الغامدي (نحو 24 ميغابايت، Opus في وعاء ogg، نفس ملفّات الهاتف).

**تفريقٌ لا يُخلَط:** أصوات الأذان تُشغَّل بمشغّلات النظام في `audio.ts` (تعمل والنافذة مغلقة)،
أمّا الأذكار والرقية فبالمشغّل العالميّ (`usePlayer`) عبر `gtsalat://res/` — لأنّها استماعٌ
طويلٌ يحتاج إيقافاً مؤقّتاً وشريطَ حالةٍ ظاهراً واستمراراً عبر الأقسام.

**فهرسُ المقاطع مصدرٌ واحد:** `ADHKAR_AUDIO_TRACKS` في `src/pages/more/AdhkarAudio.tsx` —
أضِف تسجيلاً هناك فيظهر في القسم تلقائياً، ويستعمله قسمُ الرقية نفسه لزرّ «استمع»
(`adhkarTrackById`) فلا يُعرَّف مقطعان بنفس الملفّ فيختلف عنوانُهما أو تتفرّق حالتُهما.
**والرقيةُ مُدرَجةٌ في «الأذكار الصوتية»** كما في الهاتف (إصلاح v1.15)، و**لكلّ مقطعٍ `section`
خاصّةٌ به** لا وجهةٌ واحدةٌ للقسم: الرقية تعيد إلى قسم الرقية، وأذكار النوم إلى بابها في حصن
المسلم.

**شريط المدّة (`AudioSeekBar`) للملفّات لا للبثّ:** يقرّر بنفسه من `duration` — الإذاعة بثٌّ
حيٌّ مدّته `Infinity` فيختفي. والاشتراك في `timeupdate` **داخله** لا في المزوّد، وإلّا أُعيد
رسم التطبيق كلّه أربع مرّاتٍ في الثانية. ويظهر في بطاقة المقطع **الجاري وحده** — أربعةُ
أشرطةٍ ساكنةٍ زينةٌ لا فائدة فيها.

**حالة الصوت تُبَثّ لا تُستطلَع:** `setAudioStateListener` في `audio.ts` ← `main.ts` يبثّ
`audio:state` ← لوحة التحكّم تُظهر **زرّ إيقاف الصوت** فوراً وتُسمّي الصوت الجاري.
(كان استطلاعاً كلّ ثانيتين يعرف «يعمل/لا يعمل» فقط.)

### نافذة الأذان بملء الشاشة (2.2)

`electron/adhan-window.ts` — نافذةٌ فوق كلّ النوافذ عند دخول الوقت (`fullscreenAdhan`).
**بلا preload ولا تكامل Node**: زرّ الإيقاف يستدعي `window.close()` وحده، والعملية الرئيسية
توقف الصوت في معالج `closed` — فلا مدخل بناءٍ ثانٍ لصفحةٍ واحدة ولا جسرٌ جديد.
تُحمَّل من `data:` فلا تسري عليها CSP الجلسة، ولها **سقفٌ زمنيّ** فلا تعلق إن تعطّل المشغّل.

### روايات النصّ القرآني (2.2)

`electron/riwaya.ts` — حفصٌ من `tafsir.json` المُضمَّن (فوريّ بلا شبكة)، وورش/قالون/الدوري
من `api.alquran.cloud` **سورةً سورة عند طلبها**، مخزَّنةً في `userData/riwaya_text/{slug}/{surah}.json`
(**نفس تخطيط الهاتف**). الجلب **في العملية الرئيسية** كـ`updates.ts` — فلا يُوسَّع `connect-src`.
وإن تعذّر الجلب يبقى نصّ حفص معروضاً لا صفحةٌ بيضاء. واختيارُ الرواية **يُبدّل القارئ** إلى
قارئٍ بها، وإلّا قرأ بحفصٍ ونصُّه ورش.

### تذكيرات السُّنن الموسميّة (2.2)

`sunnahReminderFor()` في `hijri.ts` (وحدةٌ بلا تبعيات) — **مطابقةٌ لـ`SunnahReminders.kt`**
نصّاً وأولويّةً: مناسبةٌ هجريّةٌ خاصّة تسبق السنّة الأسبوعيّة، **ورسالةٌ واحدةٌ في اليوم**.
تمرّ عبر `fireReminderOnce` نفسها فتَرِث نافذة السماح (ساعة).

### التطبيع العربيّ للبحث (2.2)

`normalizeArabic` **في `quran.ts` وحدها** — كان مكرَّراً ثلاث مرّات (`content.ts` ·
`Quran.tsx` · `TextContent.tsx`) فتفرّقت النتائج. وهو الآن **مطابقٌ لـ`Quran.normalize`**
في الهاتف: يحذف الشكلات والتطويل، ويوحّد الألف والياء والواو والتاء المربوطة، ثمّ **يحذف
الهمزة والألف نفسها**. ولهذا **كلّ باحثٍ يجب أن يحرس الاستعلام القصير** — «ا» وحدها تصير
فراغاً فتطابق كلّ شيء (الحرّاس قائمةٌ: `!q` أو `q.length < 2`).

### الأقسام المرقَّاة والمفضّلة (2.2)

قسمٌ كان بطاقةً في «المزيد» ثمّ صار أساسيّاً في الشريط (القرآن في 2.1) **يبقى مخزَّناً في
`favoriteSections`** فيشغل خانةً من الثلاث لا تُعرَض — فيظنّ المستخدم أنّ حقّه خانتان.
`App.tsx` يُنظّف المجهول مرّةً عند الإقلاع، و`More.tsx` يعدّ المعروف وحده.
**كلّ ترقيةٍ لقسمٍ مستقبلاً تمرّ بهذا التنظيف من تلقائها.**
...
| تذكيرات السُّنن | `sunnahReminderFor` في `hijri.ts` | `SunnahReminders.kt` |
| تطبيع البحث | `normalizeArabic` في `quran.ts` | `Quran.normalize` |
| تخطيط كاش الروايات | `userData/riwaya_text/{slug}/{surah}.json` | `filesDir/riwaya_text/…` |
